...
_.contains = function(collection, target) {
    ...
    _.each(collection, function(item){
        result = item === target ? true: result;
    });
    return result;
};
...
_.reduce = function(collection, iterator, accum) {
    var current = accum;
    var iteratorAdjust = 0;
    var collecttion2 = collection.slice(0);
    if (current === undefined) {
        iteratorAdjust = 1;
        current = collection[0];
        collecttion2 = collection.slice(1);
    }
    _.each(collecttion2, function(value, position, collection2){
        current = iterator(current, value, position + iteratorAdjust);
    });
    return current;
};
...
_.defaults = function(obj1, obj2) {
  ...
  _.each(keys2, function(value, key, collection) {
